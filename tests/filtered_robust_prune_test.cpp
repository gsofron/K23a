// FilteredRobustPrune should obviously pass all of the RobustPrune tests
// FilteredRobustPrune's only goal is to modify the graph by setting at most R out-neighbors for a point p while
// also taking into consideration the filters. This, however, doesn't guarantee that neighbors share the same
// filter, so there are no more conditions to evaluate other than the ones already found in robust_prune()
...
// General test for filtered_robust_prune
void test_filtered_robust_prune_general(void) {
    ...
    for (int i = 0 ; i < NUM_OF_VECS ; i++) {
        ...
        filtered_robust_prune(g, vectors, i, empty, A, R);
...
// Test case: V = {}
void test_filtered_robust_prune_empty_set(void) {
    std::srand(unsigned(std::time(0)));
    ...
    for (int i = 0 ; i < NUM_OF_VECS ; i++) {
        ...
        std::set<std::pair<float, int>> empty;
        filtered_robust_prune(g, vectors, i, empty, A, 1);
...
void test_filtered_robust_prune_full_set(void) {
    std::srand(unsigned(std::time(0)));
    ...
    for (int i = 0 ; i < NUM_OF_VECS ; i++) {
        ...
        filtered_robust_prune(g, vectors, i, s2, A, 1);
...
TEST_LIST = {
    { "test_filtered_robust_prune_general", test_filtered_robust_prune_general },
    { "test_filtered_robust_prune_empty_set", test_filtered_robust_prune_empty_set },
    { "test_filtered_robust_prune_full_set", test_filtered_robust_prune_full_set },
